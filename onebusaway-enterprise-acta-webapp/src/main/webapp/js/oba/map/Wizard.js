...
OBA.Wizard = function(routeMap) {	
	...
	var wizard = jQuery("#wizard"),
		...
		mapHeader = jQuery("div#map_header > *"),
		formElement = jQuery("input[name=q]"),
		results = jQuery("#matches"),
		theWindow = jQuery(window);	
	
	var search_title = getValueFor('search_title'),
		search_text = getValueFor('search_text');
			//'<p>Type a bus route, <a rel="popover" id="stop_code_popup">stopcode</a> or nearby intersection in the search box & press enter.</p><br /><p>Or keep zooming the map in by double clicking on a location.</p>',
		
		loading_text = '<img class="loading_img" src="css/map/img/wizard/loading.gif" style="vertical-align:-2px;" /> Loading bus stop info . . .';
			
		direction_title = getValueFor('direction_title'),
		direction_text = getValueFor('direction_text'),
		...
		zoomMap_text = getValueFor('zoomMap_text'),//'<span class="text_span">Try a search in the left search bar or keep zooming the map in until you see bus stop poles.</span>',
		
		follow_text = getValueFor('follow.instructions'),
		...
		share_title = getValueFor('share_title'),//'Copy this link',
		share_text_prefix = getValueFor('share_text'),'<form><input id="url" type="text" size="30" style="font-weight:bold;height:20px;width=250px;" value="http://web.mta.info/bustime',
		...
		disambiguation_title = getValueFor('disambiguation_title'),// 'Find a location',
		disambiguation_text =  getValueFor('disambiguation_text'),//'Hover over these locations to see them on the map, click to zoom in. If none are what you were looking for, try making your search <a rel="popover" id="more_specific_popup">more specific</a>.',
	...
	wizard_start.popover({
		...
		trigger: 'manual',
		left: popover_left,
		offset: mapHeader.offset().top + formElement.height()
	});
	...
	function noResultResponse() {
		alert('noResultResponse');
		hideSearchPopover();
		hideDidyoumeanPopover();
		
		
		jQuery("#wizard_inuse .text_span").html(getValueFor('noresult_text'));
	}
	...
	*/
	...
	wizard_share.popover({
		...
		delayIn: 50,
		delayOut: 100,
		fallback: 'Copy this URL: <span style="font-weight:bold;text-decoration:none">http://web.mta.info/bustime' + getSearchInputVal() + '</span>',
		html: true,
		live: false,
		offset: 10,
		placement: 'above',
